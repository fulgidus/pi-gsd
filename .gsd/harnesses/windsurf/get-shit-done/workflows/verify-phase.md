...
<required_reading>
@.windsurf/get-shit-done/references/verification-patterns.md
@.windsurf/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node ".windsurf/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See .windsurf/get-shit-done/templates/verification-report.md for complete template.
</step>
