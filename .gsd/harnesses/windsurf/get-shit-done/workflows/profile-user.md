...
Key references:
- @.windsurf/get-shit-done/references/ui-brand.md (display patterns)
- @.windsurf/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.windsurf/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".windsurf/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".windsurf/get-shit-done/USER-PROFILE.md" ".windsurf/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .windsurf/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .windsurf/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .windsurf/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.windsurf/get-shit-done/references/user-profiling.md`
...
Reference: @.windsurf/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .windsurf/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .windsurf/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .windsurf/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .windsurf/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global .windsurf/rules/" -- "Add profile to .windsurf/.windsurf/rules/ for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .windsurf/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .windsurf/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .windsurf/commands/gsd/dev-preferences.md"

**For .windsurf/rules/ profile section (if selected):**

```bash
node .windsurf/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .windsurf/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to .windsurf/.windsurf/rules/"
...
```bash
BACKUP_PATH=".windsurf/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .windsurf/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .windsurf/commands/gsd/dev-preferences.md
  ✓ .windsurf/rules/ section       .windsurf/rules/
  ✓ Global .windsurf/rules/        .windsurf/.windsurf/rules/
```
