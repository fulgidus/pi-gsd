...
```bash
INIT=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" init progress)
...
- `STATE=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node ".windsurf/get-shit-done/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node ".windsurf/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
