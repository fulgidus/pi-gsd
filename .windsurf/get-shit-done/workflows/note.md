...
- **Global scope**: `.windsurf/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `.windsurf/notes/`)
...
2. Glob `.windsurf/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `.windsurf/notes/` — works in any directory
